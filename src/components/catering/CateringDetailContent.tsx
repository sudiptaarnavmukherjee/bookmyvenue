...
export default function CateringDetailContent({ caterer }: { caterer: CatererData }) {
  ...
  const trust = assessCatererTrust({
    hasCoverImage: caterer.images.length > 0,
    imagesCount: caterer.images.length,
          ...
          <div className="flex items-center justify-between mt-3 mb-1">
            <div className="flex gap-2 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1.5 rounded-full bg-gray-100 px-3 py-1 text-xs font-medium text-gray-600 border border-gray-200">
                {getLastUpdatedLabel(caterer.updatedAt)}
              </span>
              {caterer.isPureVeg && (
            ...
            {showPriceConfidenceHelp && (
              <p className="mt-2 text-xs text-gray-600">
                {getPriceConfidenceExplanation(trust.priceConfidence, "caterer")}
              </p>
            )}
